...
def parse_testcaselocation(testcaselocation):
    ...
    if len(testcaselocation) == 0 or  len(testcaselocation) == 1:
        if testcaselocation == [] or (testcaselocation[0] == 'test_cases' and testcaselocation[-1] == 'test_cases'):
            ...
            for line in lines:
                if str(line).startswith('#'):  # delete the commented lines
                    lines.remove(line)
                else:
...
def summary(result_lists,tc_name = ''):
    ...
    global_variables.set_value('dummy_length2',dummy_length2)         # update dummy_length2
...
def statistics():
    ...
    dummy_length2 = global_variables.get_value('dummy_length2')
    ...
    if imsanity_summary_exist:  # imsanity-summary.log exists
        ...
        for line in lines:
            if '[PASS]' in line:
                ...
                basic_class.mylogger_summary.yes(real_testcase+' '+'.'*dummy_length2+' '+tmp_out[3]+' '+tmp_out[4])                       
            elif '[FAIL]' in line:
                imsanity_total += 1
                imsanity_fail += 1
                tmp_out = line.split()
                real_testcase = 'Imsanity-'+tmp_out[0]   
                basic_class.mylogger_summary.no(real_testcase+' '+'.'*dummy_length2+' '+tmp_out[3]+' '+tmp_out[4])
            else:
                pass  #skip          
    else:                       # imsanity-summary.log not exists
        basic_class.mylogger_record.info('No imsanity summary log find, skip it.')   
    
    if imsanity_summary_exist:  # IC_Summary.log exists
        pass    
    else:                       # IC_Summary.log not exists
    ...
    basic_class.mylogger_summary.yes('\nTotal number of Test Cases: '+str(total_testcases_num+imsanity_total))
    basic_class.mylogger_summary.yes('PASS:                       '+str(passed_testcases_num+imsanity_pass))
    basic_class.mylogger_summary.yes('FAIL:                       '+str(failed_testcases_num+imsanity_fail))
    ...
    basic_class.mylogger_recordnf.info('=============================================') 
    basic_class.mylogger_recordnf.info('Total number of Test Cases: '+str(total_testcases_num+imsanity_total))
    basic_class.mylogger_recordnf.info('PASS:                       '+str(passed_testcases_num+imsanity_pass))
    basic_class.mylogger_recordnf.info('FAIL:                       '+str(failed_testcases_num+imsanity_fail)+'\n')          
