...
def send_notify(subject,body,attachment_name,attachment_data):
    ...
    notify_user,default_domain,mx_account,root_account,root_passwd,mx1_host1_ip,mx1_mta1_host_ip,mx1_mta1_port,rcpts=global_variables.get_values('notify_user','default_domain','mx_account','root_account','root_passwd','mx1_host1_ip','mx1_mta1_host_ip','mx1_mta1_port','rcpts')
    ...
    remote_operations.remote_operation(mx1_host1_ip,root_account,root_passwd,'su - {0} -c \'imconfcontrol -install -key \"/*/mta/relaySourcePolicy=allowAll\";imconfcontrol -install -key \"/inbound-standardmta-direct/mta/relaySourcePolicy=allowAll\"\''.format(mx_account),0)
        
    root_passwd,basic_class.mylogger_record.info('step3:create send user:'+str(notify_user+'@'+default_domain))
    remote_operations.remote_operation(mx1_host1_ip,root_account,root_passwd,'su - {0} -c \'account-create {1}@{2}   test default\''.format(mx_account,notify_user,default_domain),1,'Mailbox Created Successfully',1)
    ...
    fast_send_mail(mx1_mta1_host_ip,mx1_mta1_port,notify_user+'@'+default_domain,rcpts.split(' '),subject=subject,body=body,attachment_name=attachment_name,attachment_data=attachment_data)
    time.sleep(5)
    
    root_passwd,basic_class.mylogger_record.info('step5:disable smtprelay:')
    remote_operations.remote_operation(mx1_host1_ip,root_account,root_passwd,'su - {0} -c \'imconfcontrol -install -key \"/*/mta/relaySourcePolicy=denyAll\";imconfcontrol -install -key \"/inbound-standardmta-direct/mta/relaySourcePolicy=denyAll\"\''.format(mx_account),0)

    root_passwd,basic_class.mylogger_record.info('step6:delete send user:test@openwave.com')
    remote_operations.remote_operation(mx1_host1_ip,root_account,root_passwd,'su - {0} -c \'account-delete {1}@{2}\''.format(mx_account,notify_user,default_domain),1,'Mailbox Deleted Successfully',1)
