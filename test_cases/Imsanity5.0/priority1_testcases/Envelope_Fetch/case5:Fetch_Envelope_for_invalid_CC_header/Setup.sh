...
prints "CASE 5 :  FOR INVALID CC HEADER " "Fetch_Envelope_for_invalid_CC_header" 

echo "From: $MAILFROM@${default_domain} "  > message.tmp 
echo "To: $Sanityuser@${default_domain}; anyuser " >> message.tmp
echo "CC: unknown" >> message.tmp
echo "Subject: Test mail" >> message.tmp
echo >> message.tmp
echo "This is a test mail ..."  >> message.tmp

msg=`cat message.tmp`
prints "Sending mail to $Sanityuser" "Fetch_Envelope_for_invalid_CC_header" 
