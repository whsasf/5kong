...
perl auth_plain_base64.pl >auth_plain_base64.tmp
auth_plain_base64=$(cat auth_plain_base64.tmp)
...
outflag=$(grep -i "Authentication successful"  auth_plain.tmp |wc -l)
echo "########## outflag=$outflag" >>debug.log
if [ "$outflag" -eq 1 ];then
	prints "auth_plain_success successfully"  "SMTP_auth_plain_success" 2
	Result=0
else
	prints "auth_plain_success not successful" "SMTP_auth_plain_success" 1
  Result=1
fi
summary "SMTP_AUTH:auth_plain_success" $Result