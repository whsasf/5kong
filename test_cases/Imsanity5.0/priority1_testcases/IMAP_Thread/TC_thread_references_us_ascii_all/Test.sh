...
if [ $? -ne 0 ];then
    imap_thread "$Sanityuser" "INBOX" "REFERENCES US-ASCII" "all"
    ...
    then
        prints "IMAP THREAD for $Sanityuser is successful" "TC_thread_references_us_ascii_all" "2"
        Result="0"
    else
        prints "-ERR IMAP THREAD for $Sanityuser is unsuccessful" "TC_thread_references_us_ascii_all" "1"
        Result="1"
    fi
else
