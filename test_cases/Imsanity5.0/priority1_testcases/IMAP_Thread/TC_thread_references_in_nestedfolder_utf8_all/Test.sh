...
if [ $? -ne 0 ];then
    imap_thread "$Sanityuser" "$folder" "REFERENCES UTF-8" "all"
    ...
    then
        prints "IMAP THREAD for $Sanityuser is successful" "TC_thread_references_in_nestedfolder_utf8_all" "2"
        Result="0"
    else
        prints "-ERR IMAP THREAD for $Sanityuser is unsuccessful" "TC_thread_references_in_nestedfolder_utf8_all" "1"
        Result="1"
    fi
else
