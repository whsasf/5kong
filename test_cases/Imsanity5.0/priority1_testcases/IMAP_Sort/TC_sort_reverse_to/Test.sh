...
imap_sort "$Sanityuser" "INBOX" "reverse to"
...
then
	prints "IMAP SORT for $imapUser is successful" "TC_sort_reverse_to" "2"
	Result="0"
else
	prints "-ERR IMAP SORT for $imapUser is unsuccessful" "TC_sort_reverse_to" "1"
	Result="1"
fi
summary "IMAP:TC_sort_reverse_to" $Result