...
function create_headerfiles(){
	echo "From: <info@owmessaging.com>" > header1.tmp
  echo "Subject: Welcome to Synchronoss Email" >> header1.tmp
  echo -e "Message-ID: <Welcome>\n" >> header1.tmp 
  echo "Welcome to Synchronoss Email by Synchronoss Technologies, Inc." >> header1.tmp
  echo "This is your first electronic mail message." >> header1.tmp
  echo "May you have many more." >> header1.tmp
  
  
  echo "To: big3 <xx1@${default_domain}>" > header2.tmp
  echo "From: gig1 <xx1@${default_domain}>" >> header2.tmp
  echo -e "Subject: 2\n" >> header2.tmp
  echo "111111111111111111" >> header2.tmp
  
  echo "From: gig5 <xx1@${default_domain}>" > header3.tmp
  echo "bcc: gig6 <xx1@${default_domain}>" >> header3.tmp
  echo -e "Subject: 3\n" >> header3.tmp
  echo "6666666666666666666666" >> header3.tmp
  
  echo "To: big1 <xx1@${default_domain}>" > header4.tmp
  echo -e "Subject: 4\n" >> header4.tmp
  echo "999999999999999999999999999" >> header4.tmp
  
  echo "To: big7 <xx1@${default_domain}>" > header5.tmp
  echo "From: gig7 <xx1@${default_domain}>" >> header5.tmp
  echo -e "Subject: 5\n" >> header5.tmp
  echo "7777777777777777777" >> header5.tmp
  
  echo "To: big1 <xx1@${default_domain}>" > header6.tmp
  echo "From: gig3 <xx1@${default_domain}>" >> header6.tmp
  echo "Subject: 6\n" >> header6.tmp
  echo "2222222222222222222" >> header6.tmp
  
  echo "From: gig2 <xx1@${default_domain}>" > header7.tmp
  echo "bcc: gig2 <xx1@${default_domain}>" >> header7.tmp
  echo -e "Subject: 7\n" >> header7.tmp
  echo "333333333333333333333" >> header7.tmp
  
  echo "bcc: gig4 <xx1@${default_domain}>" > header8.tmp
  echo -e "Subject: 8\n" >> header8.tmp
  echo "4444444444444444444444" >> header8.tmp
}
function end_time_tc() {
...
function summary() {
		...
		then 
			...
			if [ "$value" == "0" ]
			then
				printf "$func_name :-%"$count"s%s\n" "....................:" " [PASS] [$interval_time_total_tc]" >> summary.log
				Pass=$(($Pass+1))
				TotalTestCases=$(($TotalTestCases+1))
			else
				printf "$func_name :-%"$count"s%s\n" "....................:" " [FAIL] [$interval_time_total_tc] $ITS_no" >> summary.log
				Fail=$(($Fail+$value))
