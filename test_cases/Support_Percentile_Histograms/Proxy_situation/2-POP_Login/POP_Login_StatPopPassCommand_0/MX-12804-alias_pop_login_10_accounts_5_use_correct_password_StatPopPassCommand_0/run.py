...
for i in range(1,6): 
    ...
    try:
        mxpop1.pop_login('u'+str(i)+'@'+mx2_default_domain,test_account_base+str(i)) # using correct passwd
...
for i in range(6,11): 
    ...
    try:
        mxpop2.pop_login('u'+str(i)+'@'+mx2_default_domain,'password') # using wrong passwd :password here
    except:
        basic_class.mylogger_record.error('pop alias login fail')
    mxpop2.pop_quit()
