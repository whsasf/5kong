...
for i in range(1,6): 
    ...
    try:
        mximap1.imap_authenticate('u'+str(i)+'@'+mx2_default_domain,test_account_base+str(i)) # using correct passwd
...
for i in range(6,11): 
    ...
    try:
        mximap2.imap_authenticate('u'+str(i)+'@'+mx2_default_domain,'password') # using wrong passwd :password here
    except:
        basic_class.mylogger_record.error('imap alias auth_plain fail')
    mximap2.imap_logout()
