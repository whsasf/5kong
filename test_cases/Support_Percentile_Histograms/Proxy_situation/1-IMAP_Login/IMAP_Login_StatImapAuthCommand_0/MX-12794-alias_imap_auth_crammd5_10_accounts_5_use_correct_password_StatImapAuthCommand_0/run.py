...
for i in range(1,6): 
    ...
    try:
        mximap1.imap_auth_crammd5('u'+str(i)+'@'+default_domain,test_account_base+str(i)) # using correct passwd
        basic_class.mylogger_record.info('imap alias crammd5 auth success')
    except:
        basic_class.mylogger_record.error('imap alias crammd5 auth fail')
    mximap1.imap_select()
    mximap1.imap_logout()
...
for i in range(6,11): 
    ...
    try:
        mximap2.imap_auth_crammd5('u'+str(i)+'@'+default_domain,'password') # using wrong passwd :password here
    except:
        basic_class.mylogger_record.error('imap alias crammd5 auth fail')
    mximap2.imap_logout()
...
imapserv_stat_content1 = remote_operations.remote_operation(mx1_host1_ip,'su - {0} -c "cat log/imapserv.stat|grep StatImapAuthCommand"'.format(mx_account),root_account,root_passwd,0)
result_lists1 = stat_statistics.stat_statistic(imapserv_stat_content1,'[0]','StatImapAuthCommand',10)

imapserv_stat_content2 = remote_operations.remote_operation(mx2_host1_ip,'su - {0} -c "cat log/imapserv.stat|grep StatImapAuthCommand"'.format(mx_account),root_account,root_passwd,0)
