...
for i in range(1,6): 
    ...
    try:
        mximap1.imap_auth_crammd5('u'+str(i)+'@'+mx1_default_domain,test_account_base+str(i)) # using correct passwd
...
for i in range(6,11): 
    ...
    try:
        mximap2.imap_auth_crammd5('u'+str(i)+'@'+mx1_default_domain,'password') # using wrong passwd :password here
    except:
        basic_class.mylogger_record.error('imap alias crammd5 auth fail')
    mximap2.imap_logout()
...
result_lists = stat_statistics.stat_statistic(imapserv_stat_content,'[0]','StatImapAuthCommand',10)
