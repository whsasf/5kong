...
then
	# Checking for how many append commands are occuring between close command and close server output.
	...
	TC1_APPEND_COUNT=$(sed -e '/CLOSEINBOX close/,/CLOSEINBOX OK  CLOSE/!d' TC_logs.txt | head -$TC1_Head_Value | tail -$TC1_Tail_Value | grep -i append | wc -l)
	echo "Total number of append commands occuring between close command and close server output ==> "$TC1_APPEND_COUNT >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	then
		echo "TC_2_1_47 PASSED: Concurrency Acheieved in 'IMAP-append' and 'IMAP-close' commands."
		echo "TC_2_1_47 PASSED: Concurrency Acheieved in 'IMAP-append' and 'IMAP-close' commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "There were $TC1_APPEND_COUNT APPEND operation calls between a single close operation."
		echo "There were $TC1_APPEND_COUNT APPEND operation calls between a single close operation." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo
		echo "Test Completed."
		echo "Test Completed." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "TC_2_1_47 ==> PASSED" >> $PATH_TO_SCRIPTS/Logs/IC_Summary.log
		echo "=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	else
		echo "TC_2_1_47 FAILED."
		echo "TC_2_1_47 FAILED." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "Reason: Commands fired between close commands are not append commands."
		echo "Reason: Commands fired between close commands are not append commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		$PATH_TO_SCRIPTS/TC_2_1_47/IC_TC.sh
	fi
else
	echo "TC_2_1_47 Failed"
	echo "TC_2_1_47 Failed" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	echo "Running TC_2_1_47 again..."
	echo "Running TC_2_1_47 again..." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	$PATH_TO_SCRIPTS/TC_2_1_47/IC_TC.sh
fi
