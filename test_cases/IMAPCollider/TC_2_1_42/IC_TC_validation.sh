...
then
	# Checking for how many store commands are occuring between append command and append server output.
	...
	TC1_STORE_COUNT=$(sed -e '/APPENDINBOX append/,/APPENDINBOX OK/!d' TC_logs.txt | head -$TC1_Head_Value | tail -$TC1_Tail_Value | grep -i SETSTORE | wc -l)
	echo "Total number of store commands occuring between append command and append server output ==> "$TC1_STORE_COUNT >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	then
		echo "TC_2_1_42 PASSED: Concurrency Acheieved in 'IMAP-store' and 'IMAP-append' commands."
		echo "TC_2_1_42 PASSED: Concurrency Acheieved in 'IMAP-store' and 'IMAP-append' commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "There were $TC1_STORE_COUNT store operation calls between a single append operation."
		echo "There were $TC1_STORE_COUNT store operation calls between a single append operation." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo
		echo "Test Completed."
		echo "Test Completed." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "TC_2_1_42 ==> PASSED" >> $PATH_TO_SCRIPTS/Logs/IC_Summary.log
		echo "=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	else
		echo "TC_2_1_42 FAILED."
		echo "TC_2_1_42 FAILED." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "Reason: Commands fired between append commands are not store commands."
		echo "Reason: Commands fired between append commands are not store commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		$PATH_TO_SCRIPTS/TC_2_1_42/IC_TC.sh
	fi
else
	echo "TC_2_1_42 Failed"
	echo "TC_2_1_42 Failed" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	echo "Running TC_2_1_42 again..."
	echo "Running TC_2_1_42 again..." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	$PATH_TO_SCRIPTS/TC_2_1_42/IC_TC.sh
fi
