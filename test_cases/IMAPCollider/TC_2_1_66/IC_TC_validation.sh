...
then
	# Checking for how many list commands are occuring between fetch command and fetch server output.
	...
	TC1_LIST_COUNT=$(sed -e '/FETCHINBOX fetch/,/FETCHINBOX OK  FETCH/!d' TC_logs.txt | head -$TC1_Head_Value | tail -$TC1_Tail_Value | grep -i list | wc -l)
	echo "Total number of list commands occuring between fetch command and fetch server output ==> "$TC1_LIST_COUNT >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	then
		echo "TC_2_1_66 PASSED: Concurrency Acheieved in 'IMAP-fetch' and 'IMAP-list' commands."
		echo "TC_2_1_66 PASSED: Concurrency Acheieved in 'IMAP-fetch' and 'IMAP-list' commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "There were $TC1_LIST_COUNT LIST operation calls between a single fetch operation."
		echo "There were $TC1_LIST_COUNT LIST operation calls between a single fetch operation." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo
		echo "Test Completed."
		echo "Test Completed." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "TC_2_1_66 ==> PASSED" >> $PATH_TO_SCRIPTS/Logs/IC_Summary.log
		echo "=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	else
		echo "TC_2_1_66 FAILED."
		echo "TC_2_1_66 FAILED." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "Reason: Commands fired between fetch commands are not list commands."
		echo "Reason: Commands fired between fetch commands are not list commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		$PATH_TO_SCRIPTS/TC_2_1_66/IC_TC.sh
	fi
else
	echo "TC_2_1_66 Failed"
	echo "TC_2_1_66 Failed" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	echo "Running TC_2_1_66 again..."
	echo "Running TC_2_1_66 again..." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	$PATH_TO_SCRIPTS/TC_2_1_66/IC_TC.sh
fi
