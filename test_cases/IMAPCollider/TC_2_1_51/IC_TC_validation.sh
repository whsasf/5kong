...
then
	# Checking for how many copy commands are occuring between append command and append server output.
	...
	TC1_COPY_COUNT=$(sed -e '/APPENDINBOX append/,/APPENDINBOX OK/!d' TC_logs.txt | head -$TC1_Head_Value | tail -$TC1_Tail_Value | grep -i COPYCOPY | wc -l)
	echo "Total number of copy commands occuring between append command and append server output ==> "$TC1_COPY_COUNT >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	then
		echo "TC_2_1_51 PASSED: Concurrency Acheieved in 'IMAP-copy' and 'IMAP-append' commands."
		echo "TC_2_1_51 PASSED: Concurrency Acheieved in 'IMAP-copy' and 'IMAP-append' commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "There were $TC1_COPY_COUNT copy operation calls between a single append operation."
		echo "There were $TC1_COPY_COUNT copy operation calls between a single append operation." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo
		echo "Test Completed."
		echo "Test Completed." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "TC_2_1_51 ==> PASSED" >> $PATH_TO_SCRIPTS/Logs/IC_Summary.log
		echo "=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	else
		echo "TC_2_1_51 FAILED."
		echo "TC_2_1_51 FAILED." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "Reason: Commands fired between append commands are not copy commands."
		echo "Reason: Commands fired between append commands are not copy commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		$PATH_TO_SCRIPTS/TC_2_1_51/IC_TC.sh
	fi
else
	echo "TC_2_1_51 Failed"
	echo "TC_2_1_51 Failed" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	echo "Running TC_2_1_51 again..."
	echo "Running TC_2_1_51 again..." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	$PATH_TO_SCRIPTS/TC_2_1_51/IC_TC.sh
fi
