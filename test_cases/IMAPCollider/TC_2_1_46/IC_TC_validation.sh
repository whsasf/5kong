...
then
	# Checking for how many append commands are occuring between delete command and delete server output.
	...
	TC1_APPEND_COUNT=$(sed -e '/DELETEDEL delete/,/DELETEDEL OK  DELETE/!d' TC_logs.txt | head -$TC1_Head_Value | tail -$TC1_Tail_Value | grep -i append | wc -l)
	echo "Total number of append commands occuring between delete command and delete server output ==> "$TC1_APPEND_COUNT >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	then
		echo "TC_2_1_46 PASSED: Concurrency Acheieved in 'IMAP-delete' and 'IMAP-append' commands."
		echo "TC_2_1_46 PASSED: Concurrency Acheieved in 'IMAP-delete' and 'IMAP-append' commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "There were $TC1_APPEND_COUNT APPEND operation calls between a single delete operation."
		echo "There were $TC1_APPEND_COUNT APPEND operation calls between a single delete operation." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo
		echo "Test Completed."
		echo "Test Completed." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "TC_2_1_46 ==> PASSED" >> $PATH_TO_SCRIPTS/Logs/IC_Summary.log
		echo "=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=X=" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	else
		echo "TC_2_1_46 FAILED."
		echo "TC_2_1_46 FAILED." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		echo "Reason: Commands fired between delete commands are not APPEND commands."
		echo "Reason: Commands fired between delete commands are not APPEND commands." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
		$PATH_TO_SCRIPTS/TC_2_1_46/IC_TC.sh
	fi
else
	echo "TC_2_1_46 Failed"
	echo "TC_2_1_46 Failed" >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	...
	echo "Running TC_2_1_46 again..."
	echo "Running TC_2_1_46 again..." >> $PATH_TO_SCRIPTS/Logs/IC_Debug.log
	$PATH_TO_SCRIPTS/TC_2_1_46/IC_TC.sh
fi
