...
remote_operations.remote_operation(mx1_host1_ip,'su - {0} -c \'imconfcontrol -install -key \"/*/common/hostInfo=blobtier=Cassandra:blobcluster:9162\";imconfcontrol -install -key \"/site1-inbound-standardmta-direct/mta/subAddressAllowedIPs=127.0.0.1\n10.37.2.214\";imconfcontrol -install -key \"/*/mta/subAddressAllowedIPs=127.0.0.1\n10.37.2.214\";imconfcontrol -install -key \"/*/common/messageBodyEncryptionEnabled=false\";imconfcontrol -install -key \"/*/mss/compressionEnabled=false\"\''.format(mx_account),root_account,root_passwd,0)
...
basic_class.mylogger_record.info('step3:create costomerfolder folder on testuser1 and append 1 message in it')

mximap1 = imap_operations.IMAP_Ops(mx1_imapserv_host1_ip,mx1_imapserv_host1_imap4Port)
mximap1.imap_login(test_account_base+str(1),test_account_base+str(1))  # testuser1
mximap1.imap_create('customerfolder')
mximap1.imap_select('customerfolder')
mximap1.imap_append(mailbox='customerfolder')
mximap1.imap_logout()

#basic_class.mylogger_record.info('step4:deliever 1 message from testuser2 to testuser1 to INBOX')
